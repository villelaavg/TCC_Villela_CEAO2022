...
intervalo_distancias = [40, 100, 150, 360]
intervalo_angulos = [i for i in range(0,359,15)]
intervalo_apg = [40, 65, 150, 565]
lat_0 = 0
lon_0 = 0
...
# print(dados.columns)
apg_40 = dados.iloc[:, :9]
excl_65 = ['lkp_40', 'lat_lkp_40', 'lon_lkp_40',
       'destino_40', 'lat_destino_40', 'lon_destino_40', 'lkp_150', 'lat_lkp_150', 'lon_lkp_150',
       'destino_150', 'lat_destino_150', 'lon_destino_150','lkp_565', 'lat_lkp_565', 'lon_lkp_565',
       'destino_565', 'lat_destino_565', 'lon_destino_565']

excl_150 = ['lkp_40', 'lat_lkp_40', 'lon_lkp_40', 
            'destino_40', 'lat_destino_40', 'lon_destino_40', 'lkp_65',
            'lat_lkp_65', 'lon_lkp_65', 'destino_65', 'lat_destino_65',
            'lon_destino_65','lkp_565', 'lat_lkp_565', 'lon_lkp_565',
            'destino_565', 'lat_destino_565', 'lon_destino_565']

excl_565 = ['lkp_40', 'lat_lkp_40', 'lon_lkp_40', 
            'destino_40', 'lat_destino_40', 'lon_destino_40', 'lkp_65',
            'lat_lkp_65', 'lon_lkp_65', 'destino_65', 'lat_destino_65',
            'lon_destino_65','lkp_150', 'lat_lkp_150', 'lon_lkp_150',
            'destino_150', 'lat_destino_150', 'lon_destino_150']
apg_65 = dados.drop(columns=excl_65)
apg_150 = dados.drop(columns=excl_150)
apg_565 = dados.drop(columns=excl_565)
...
apg_65.columns = [novos_nomes]
apg_150.columns = [novos_nomes]
apg_565.columns = [novos_nomes]

#%%
#Salvando os datasets
apg_40.to_excel('apg_40.xlsx')
apg_65.to_excel('apg_65.xlsx')
apg_150.to_excel('apg_150.xlsx')
apg_565.to_excel('apg_565.xlsx')
...
apg_65 = pd.read_excel('apg_65.xlsx', usecols='B:J')
apg_65 = apg_65.iloc[1:,:]
apg_65 = apg_65.reset_index(drop=True)

apg_150 = pd.read_excel('apg_150.xlsx', usecols='B:J')
apg_150 = apg_150.iloc[1:,:]
apg_150 = apg_150.reset_index(drop=True)

apg_565 = pd.read_excel('apg_565.xlsx', usecols='B:J')
apg_565 = apg_565.iloc[1:,:]
apg_565 = apg_565.reset_index(drop=True)
...
for i in range(len(apg_65)):
    #Cálculo dos ângulos
    destino = [apg_65['LAT_DESTINO'][i], apg_65['LONG_DESTINO'][i]]
    lkp = [apg_65['LAT_LKP'][i], apg_65['LONG_LKP'][i]]
    base = [apg_65['LAT_BASE'][i], apg_65['LONG_BASE'][i]]  
    param_angulos = calculo_angulos(destino, lkp)

    #Geração dos datasets
    nome_dataset = apg_65.OBJETO_BUSCA[i]
    print(f'Dataset: {nome_dataset}')
    lkp_sup, lkp_inf, destino_sup, destino_inf = pontos_rebatidos(destino, lkp)
    df_sup = gera_dataset(destino_sup, lkp_sup, param_angulos)
    df_inf = gera_dataset(destino_inf, lkp_inf, param_angulos)
    ...
    caminhos['dist_reposicionamento'] = lista_repos
    caminhos['dist_total'] = caminhos.dist_reposicionamento + caminhos.distancia
    caminhos.to_excel(f'resultados_apg_65/matriz_{nome_dataset}.xlsx', index=False)
...
resultados_apg_65 = pd.DataFrame({'dataset': apg_65.OBJETO_BUSCA,
                           'qtd rotas ideais': qtd_min,
                            'rota ideal': rota_min,                            
                            'dist rota ideal (nm)': lista_min,
                            'qtd maiores rotas': qtd_max,
...
print(resultados_apg_65)

#%%
#Salvando o DataFrame 'resultados'
resultados_apg_65.to_excel('resultados_apg_65_full.xlsx', index=False)

#%%
print()
print('Rotas Mínimas:')
print(resultados_apg_65['rota ideal'].value_counts())
print()

print('Rotas Máximas:')
print(resultados_apg_65['maior rota'].value_counts())
...
for i in range(len(apg_150)):
    #Cálculo dos ângulos
    destino = [apg_150['LAT_DESTINO'][i], apg_150['LONG_DESTINO'][i]]
    lkp = [apg_150['LAT_LKP'][i], apg_150['LONG_LKP'][i]]
    base = [apg_150['LAT_BASE'][i], apg_150['LONG_BASE'][i]]  
    param_angulos = calculo_angulos(destino, lkp)

    #Geração dos datasets
    nome_dataset = apg_150.OBJETO_BUSCA[i]
    print(f'Dataset: {nome_dataset}')
    lkp_sup, lkp_inf, destino_sup, destino_inf = pontos_rebatidos(destino, lkp)
    df_sup = gera_dataset(destino_sup, lkp_sup, param_angulos)
    df_inf = gera_dataset(destino_inf, lkp_inf, param_angulos)
    ...
    caminhos['dist_reposicionamento'] = lista_repos
    caminhos['dist_total'] = caminhos.dist_reposicionamento + caminhos.distancia
    caminhos.to_excel(f'resultados_apg_150/matriz_{nome_dataset}.xlsx', index=False)
...
resultados_apg_150 = pd.DataFrame({'dataset': apg_150.OBJETO_BUSCA,
                           'qtd rotas ideais': qtd_min,
                            'rota ideal': rota_min,                            
                            'dist rota ideal (nm)': lista_min,
                            'qtd maiores rotas': qtd_max,
...
print(resultados_apg_150)

#%%
#Salvando o DataFrame 'resultados'
resultados_apg_150.to_excel('resultados_apg_150_full.xlsx', index=False)

#%%
print()
print('Rotas Mínimas:')
print(resultados_apg_150['rota ideal'].value_counts())
print()

print('Rotas Máximas:')
print(resultados_apg_150['maior rota'].value_counts())
...
for i in range(len(apg_565)):
    #Cálculo dos ângulos
    destino = [apg_565['LAT_DESTINO'][i], apg_565['LONG_DESTINO'][i]]
    lkp = [apg_565['LAT_LKP'][i], apg_565['LONG_LKP'][i]]
    base = [apg_565['LAT_BASE'][i], apg_565['LONG_BASE'][i]]  
    param_angulos = calculo_angulos(destino, lkp)

    #Geração dos datasets
    nome_dataset = apg_565.OBJETO_BUSCA[i]
    print(f'Dataset: {nome_dataset}')
    lkp_sup, lkp_inf, destino_sup, destino_inf = pontos_rebatidos(destino, lkp)
    df_sup = gera_dataset(destino_sup, lkp_sup, param_angulos)
    df_inf = gera_dataset(destino_inf, lkp_inf, param_angulos)
    ...
    caminhos['dist_reposicionamento'] = lista_repos
    caminhos['dist_total'] = caminhos.dist_reposicionamento + caminhos.distancia
    caminhos.to_excel(f'resultados_apg_565/matriz_{nome_dataset}.xlsx', index=False)
...
resultados_apg_565 = pd.DataFrame({'dataset': apg_565.OBJETO_BUSCA,
                           'qtd rotas ideais': qtd_min,
                            'rota ideal': rota_min,                            
                            'dist rota ideal (nm)': lista_min,
                            'qtd maiores rotas': qtd_max,
...
print(resultados_apg_565)

#%%
#Salvando o DataFrame 'resultados'
resultados_apg_565.to_excel('resultados_apg_565_full.xlsx', index=False)

#%%
print()
print('Rotas Mínimas:')
print(resultados_apg_565['rota ideal'].value_counts())
print()

print('Rotas Máximas:')
print(resultados_apg_565['maior rota'].value_counts())
